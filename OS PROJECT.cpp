...
int main()
{
	...
	{		
		b1=sjf(b1);					
	for (int i=0;i<b1.n1;i++)
	{
		cout<<"Process in batch 1: "<<display->id<<" ";
		display=display->next;
	}
	display=b2.h;
		b3=priority(b3);		
		for (int i=0;i<i<b2.n1;i++)
		{
			cout<<"Process in batch 2: "<<display->id<<" ";
			display=display->next;
		}
		display=b3.h;
		for (int i=0;i<i<b3.n1;i++)
		{
			cout<<"Process in batch 3: "<<display->id<<" ";
			display=display->next;
		}
		b1=removeprocess(b1);
		cpu=buffer1;
